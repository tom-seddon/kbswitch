...
__declspec(dllexport)
LRESULT CALLBACK KBSwitchHookProc(int nCode,WPARAM wParam,LPARAM lParam)
{
	switch(nCode)
	{
	case HCBT_CREATEWND:
		{
			...
			dprintf("%s: HCBT_CREATEWND: HWND=%p hKL=%p title=\"%s\"\n",__FUNCTION__,hWnd,g_hKL,title);

			if(g_hKL)
			{
				dprintf("%s: (posting message...)\n",__FUNCTION__);
